...
_, _, test_loader = dataloader.get_loader(root=DATA, train_transform=None, val_transform=val_transform, target_transform=target_transform,
        batch_size=loader_bsize, num_frames=NUM_FRAMES, step_size=FRAME_STEP, val_samples=loader_bsize, n_threads=NUM_THREADS, training=False, val=False, test=True)

video_files = glob.glob(os.path.join(DATA, 'test', '0', '*.gulp'))
video_ids = [os.path.basename(v).split('.')[0] for v in video_files]
